...
import { Leaf, MessageCircle, Brain, Zap } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import ChatInterface from '@/components/ChatInterface';
import QuizInterface from '@/components/QuizInterface';
import ChapterHeader from '@/components/biology/ChapterHeader';
import SummaryTab from '@/components/biology/SummaryTab';
import SimulationTab from '@/components/biology/SimulationTab';
import { BiologyChapterNotes, BiologyChapter } from '@/types/biology';

export default function BiologyChapterPage() {
  const { chapterId } = useParams();
  const { user } = useAuth();
  const { toast } = useToast();
  const [notes, setNotes] = useState<BiologyChapterNotes | null>(null);
  ...
  const fetchChapterData = async () => {
    ...
    try {
      ...
      const { data: chapterData, error: chapterError } = await supabase
        ...
        .select('*')
        .eq('biology_chapter_id', parseInt(chapterId))
        .single();
      ...
      const { data: notesData, error: notesError } = await supabase
        .from('dim_biology_chapters_notes')
        .select('*')

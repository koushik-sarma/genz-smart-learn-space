...
export default function SocialPart2() {
  const { user } = useAuth();
  const navigate = useNavigate();
  const { toast } = useToast();
  const [chapters, setChapters] = useState<SocialChapter[]>([]);
  ...
  const fetchSocialData = async () => {
    try {
      ...
      // Get the subject_id for Social Studies Part 2
      const { data: subjectData, error: subjectError } = await supabase
        .from('dim_subjects')
        .select('subject_id')
        .eq('subject_name', 'Social Studies - Part 2')
      ...
      // Fetch social science chapters based on user's class, board, and subject (part = 2)
      ...
      toast({
        title: "Error loading social science chapters",
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
        <div className="text-white text-lg">Loading social science chapters...</div>
      </div>
    );
  }
      ...
      <div className="relative z-10 p-6">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="text-white">
              <h1 className="text-3xl font-bold">Social Science - Part 2</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {chapters.map((chapter) => (
            <Card key={chapter.social_chapter_id} className="bg-white/10 backdrop-blur-xl border-white/20 hover:bg-white/15 transition-all cursor-pointer">
              <CardHeader>
                <div className="text-sm text-purple-300 mb-2">Chapter {chapter.social_chapter_id}</div>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex justify-end items-center pt-2">
                  <Button 
                    ...
                    onClick={() => navigate(`/social/chapter/${chapter.social_chapter_id}`)}
                  >
                    Start Learning
                  </Button>
                </div>
          ...
          <Card className="bg-white/10 backdrop-blur-xl border-white/20 text-center py-12">
            <CardContent>
              ...
              <div className="text-gray-300">
                Social Science Part 2 chapters for your class and board will appear here once they're available.
              </div>
            </CardContent>
          </Card>
        )}
